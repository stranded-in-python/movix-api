...
class FilmService(FilmServiceABC):
    ...
    async def get_films(self,
        ...
        films = self.storage.get_items(sort, pagination_params, genre_id, similar_to)
    ...
    async def get_by_query(self, query, pagination_params) -> list[FilmShort] | None:
        ...
        films = self.storage.get_by_query(query, pagination_params)
...
@lru_cache
def get_films_service() -> FilmService:
