...
async def test_get_by_id_not_found(client):
    ...
    response = await client.get("/api/v1/persons/3d8d9bf5-0d90-4353-88ba-4ccc5d2c07ff")
...
@pytest.mark.person_films
async def test_get_person_films_ok(client):
...
@pytest.mark.person_films
async def test_get_person_films_not_found_person(client):
    """Проверка обработки несуществующего идентификатора персоны"""
    response = await client.get(
...
@pytest.mark.person_films
async def test_get_person_films_unprocessable_person_id(client):
    """Проверка обработки некорректного формата
    идентификатора персоны"""
...
@pytest.mark.search_person
async def test_search_person_ok(client):
...
@pytest.mark.search_person
async def test_search_person_not_found(client):
